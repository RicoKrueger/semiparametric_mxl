...
from mon import monrnd, monlpdf_q, next_mu, next_iwDiagA, \
...
def estimate(
    ...
    xRnd2_transBool = np.sum(xRnd2_trans) > 0
    ...
    for i in np.arange(mcmc_iterSampleThin):
        ...
        for t in np.arange(nTakes):
            ...
            v = sim_xRnd[:,0] * paramRndPerRow[:,0] \
                + paramRndPerRow[:,1] * (sim_xRnd[:,1] + np.sum(sim_xRnd[:,2:] * paramRndPerRow[:,2:], axis = 1))
