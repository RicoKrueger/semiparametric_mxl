...
xRnd2 = np.zeros((0,0)) #np.array(xRnd)
xRnd2_valB = np.zeros((0,0)) #np.array(xRnd_valB)
xRnd2_valW = np.zeros((0,0)) #np.array(xRnd_valW)
xRnd2_trans = np.zeros((0,0)) #np.array([0, 0, 0, 0])
...
modelName = 'mvn' + '_sim' + '_S' + str(S) + '_N' + str(N) + '_T' + str(T) + '_r' + str(r)
deleteDraws = False
