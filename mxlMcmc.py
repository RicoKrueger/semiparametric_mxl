...
from mxl import corrcov, prepareData, mvnlpdf, pPredMxl, transFix, transRnd
...
def next_Omega(paramRnd, zeta, nu, iwDiagA, diagCov, nRnd, nInd):
    ...
    if diagCov: 
        Omega_tmp = np.array(Omega)
        Omega = np.diag(np.diag(Omega_tmp))
        Omega[2:,2:] = Omega_tmp[2:,2:] 
    return Omega
...
def postAna(paramName, nParam, nParam2, mcmc_nChain, mcmc_iterSampleThin, modelName):
    ...
    draws_split[:mcmc_nChain,:,:,:] = postDraws[:,:n,:,:]
    draws_split[mcmc_nChain:,:,:,:] = postDraws[:,n:,:,:]
...
def mcmcChainPredW(
    ...
    paramFix_store = None
    if nFix: paramFix_store = np.array(file['paramFix_store'])
    paramRnd_store = np.array(file['paramRnd_store'][:,:nInd,:])
    ...
    for i in np.arange(mcmc_iterSampleThin):
        if nFix: vFix = xFix @ paramFix_store[i,:]
        paramRndPerRow = np.repeat(paramRnd_store[i,:,:], rowsPerInd, axis = 0)
        vRnd = np.sum(xRnd * paramRndPerRow, axis = 1)
        pPred += pPredMxl(vFix, vRnd, map_avail_to_obs, 1, chosenIdx, nonChosenIdx)
